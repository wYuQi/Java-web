...
/**
 * Servlet implementation class PurchoseGet
 */
@WebServlet("/PurchoseGet.do")
public class PurchoseGet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//不允许超过库容量
		 session.setAttribute("enough", "");
		if(count1+num > maxcount2){
			System.out.println("超过了");
			session.setAttribute("enough", "超过最大库存量");
		}else {
			String sqlad = "insert into purchase(commodity_id,market_id,number,time) values(?,?,?,?)";
		...
		request.getRequestDispatcher("Purchose.do").forward(request, response);
	}
